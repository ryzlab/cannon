...
class CannonController:
    ...
    UP = Command("up", 0x02, True)
    DOWN = Command("down", 0x01, True)
    ...
    def __stop_callback(self):
        ...
        if self.hidraw is not None:
            self.hidraw.write([0x02, CannonController.STOP.hid_code, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    ...
    def send_command(self, command, delay=None):
        ...
        if self.thread is not None:
            ...
            self.thread = None
        if self.hidraw is not None:
            self.hidraw.write([0x02, command.hid_code, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        if delay:
