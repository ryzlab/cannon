...
public class MissileLauncher {
    private static final Logger logger = Logger.getLogger(MissileLauncher.class.getName());
     ...
     */
    public static final int CMD_UP = 0x01;

    /**
     * Command to rotate the launcher down.
     */
    public static final int CMD_DOWN = 0x02;

    /**
     * Command to rotate the launcher to the left.
     */
    public static final int CMD_LEFT = 0x04;

    /**
     * Command to rotate the launcher to the right.
     */
    public static final int CMD_RIGHT = 0x08;

    /**
     * Command to fire a missile.
     */
    public static final int CMD_FIRE = 0x10;
     ...
     *
     * @param hub The USB hub to search on.
    ...
    public static UsbDevice findMissileLauncher(UsbHub hub) {
        ...
        for (UsbDevice device : (List<UsbDevice>) hub.getAttachedUsbDevices()) {
            if (device.isUsbHub()) {
                ...
                String foundIdHex = String.format("0x%08x", desc.idVendor());
                String foundProductHex = String.format("0x%08x", desc.idProduct());
                logger.info("Found: " +
                        String.format("%04x", desc.idVendor()) + ":" +
                        String.format("%04x", desc.idProduct()) + ", looking for " +
                        String.format("%04x", VENDOR_ID) + ":" +
                        String.format("%04x", PRODUCT_ID));
